df <- read.csv("./data/tau-tau-data.csv")
report <- TRUE # FALSE

printreport <- function(net) {
    largesd <- df[df$net == net & df$metric == "largesd", ]
    d <- df[df$net == net & df$metric == "d", ]
    summary(abs(largesd$tau) - abs(d$tau))
}

plotit <- function(net) {
    imagefile <- paste0("./img/tau-tau-", net, ".pdf")
    ...
    for(i in 1:5) {
        ...
        plotdf <- df[df$n == i & df$net == net, ]# & df$model %in% selectmodels, ]    
        
        x <- abs(plotdf$tau[plotdf$metric == "largesd"])
        ...
        pchs <- as.integer(plotdf$cond[plotdf$metric == "d"]) + 20
                                        # use filled and empty to separate bparam
        ## pchs <- mapply(
        ##     function(x, y) pch_lookup[x, y],
        ##     plotdf[plotdf$metric == "d", "cond"], plotdf[plotdf$metric == "d", "bparam"]
        ## )

        plotdf$bparam <- factor(plotdf$bparam, levels = c("u", "D"))
                
        colors <- as.integer(factor(plotdf$model[plotdf$metric == "d"]))
        ## means <- aggregate(tau ~ metric + cond, data = plotdf, FUN = mean)
        bgs <- mapply(
        ...
        plot(
            ...
            bg = bgs,#sapply(colors, function(z) adjustcolor(z, alpha.f = .5)),
                                        # use marker size to mark bparam
            ## cex = (1/as.numeric(plotdf$bparam[plotdf$metric == "d"])) * 4,
            xlab = "Large SD", ylab = "",
            xlim = xlim, ylim = ylim,
        ...
        abline(a = 0, b = 1, lty = 1, col = "black", lwd = .5)
                                        ...
                                        # use x's to mark D bparam
        ## markX <- grep("D", plotdf$bparam[plotdf$metric == "d"])
        ## points(
        ##     x[markX], y[markX], col = colors[markX], pch = 4, cex = 3, lwd = 2
        ## )
                                        # mark means
        ## points(
        ##     means$tau[means$metric == "largesd"], means$tau[means$metric == "d"],
        ##     cex = 3, col = "gray", pch = 1:3, lwd = 3
        ## )

        ## mtext(LETTERS[i], line = -2.5, adj = 0.02, font = 2, cex = 2)
        mtext(paste0("(", letters[i], ")"), line = -2.5, adj = 0.02, cex = 2) # , font = 2
    }
